...
function! gnvim#hover#hide_hover()
...
function! gnvim#hover#cursor_moved()
	let l:pos = getcurpos()
	let l:cur_row = l:pos[1] - 1
	let l:cur_col = l:pos[2] - 1

	if type(s:gnvim_hover_pos) == type({})
		let l:start = s:gnvim_hover_pos['start']
		let l:end = s:gnvim_hover_pos['end']

		let l:after_start = v:false
		let l:before_end = v:false

		if l:cur_row == l:start['line']
			if l:cur_col >= l:start['character']
				let l:after_start = v:true
			endif
		endif

		if l:cur_row == l:end['line']
			if l:cur_col <= l:end['character']
				let l:before_end = v:true
			endif
		endif

		if l:after_start && l:before_end
			return
		endif
...
function! s:handle_hover(server, pos, screencol, data) abort
    ...
    if !empty(a:data['response']['result']) && !empty(a:data['response']['result']['contents'])
    	...
    	call rpcnotify(0, 'Gnvim', 'ShowHover',
    				\ l:content, l:pos[0] - line_offset, max([l:pos[1] + col_offset, 0]))
        return
    else
